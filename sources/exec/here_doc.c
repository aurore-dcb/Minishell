...
/*   Updated: 2023/09/26 11:19:53 by aducobu          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_here_doc(cmd_line *cmd, pipex *pipex, s_data *data, t_pid **pids)
{
	...
	if (!standart_input(cmd, pipex))
		return (0);
	if (g_flag == 1)
		ft_lstadd_back_file(&cmd->infile, ft_lstnew_file(open("/dev/stdout", O_RDONLY), 0, NULL));
	else
		ft_lstadd_back_file(&cmd->infile, ft_lstnew_file(pipex->here_doc_file, 0, NULL));
	close(pipex->here_doc_file);
